...
hree Elves, the badge is the only item type carried by all three Elves. That 
...
data = readlines("./Day 03 - Rucksack Reorganization/data.txt", keep = false)

function part_one(lines::Array{String})
    items = Char[]
    for line ∈ lines
        l = round(Int64, length(line) / 2)
        c = Char[]
        for i ∈ 1:l        
            contains(line[l+1:end], line[1:l][i]) && append!(c, line[1:l][i])
        end
        append!(items, unique(c))
    end

    priorities = Int64[]
    for item ∈ items
        n = convert(Int64, item)
        if n < 97
            append!(priorities, n - 65 + 27)
        else
            append!(priorities, n - 97 + 1)
        end
    end
...
function part_two(list::Array{String})
    items = Char[]
    for i ∈ range(1, length(list), step = 3)
        c = Char[]
        for ii ∈ 1:length(list[i])
            if contains(list[i+1], list[i][ii]) && contains(list[i+2], list[i][ii])
                append!(c, list[i][ii])
            end
        end
        append!(items, unique(c))
