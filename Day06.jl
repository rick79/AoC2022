...
last four characters received are jpqm, which are all different. In this case, your 
...
for line ∈ eachsplit(data, "\n", keepempty = false)
    for i ∈ 1:(length(line)-13)
        if length(unique(line[i:i+13])) == 14
            println("Part Two: match at $(i+13) !")
            break
        end
    end
end