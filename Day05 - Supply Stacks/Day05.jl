...
f = open("./Day05 - Supply Stacks/data.txt", "r")
data = read(f, String)       
input = split(data, "\n\n")
close(f)

function create_stacks(input::String)
...
stacks1 = create_stacks(String(input[1]))
stacks2 = deepcopy(stacks1)

function part_one(input::String, stacks::Dict{Char, Array})
    for operation ∈ eachsplit(input, "\n", keepempty = false)
    ...
    for key ∈ sort(collect(keys(stacks)))
        res = res * stacks[key][end]
    end
    return(res)
end

res = part_one(String(input[2]), stacks1)
println("Part One: $res")


function part_two(input::String, stacks::Dict{Char, Array})
    for operation ∈ eachsplit(input, "\n", keepempty = false)
        items = split(operation, " ")
        n = parse(Int64, items[2])
        source = only(items[4])
...
res = part_two(String(input[2]), stacks2)
